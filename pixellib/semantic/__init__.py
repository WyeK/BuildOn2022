...
class semantic_segmentation():
    ...
    def process_video_pascalvoc(self, video_path, overlay=False, frames_per_second=None, output_video_name=None):
        ...
        if output_video_name is not None:
            save_video = cv2.VideoWriter(output_video_name, cv2.VideoWriter_fourcc(*'DIVX'), frames_per_second,
                                         (width, height))
    ...
    def process_camera_pascalvoc(self, cam, overlay=False, check_fps=False, frames_per_second=None,
                                 output_video_name=None, show_frames=False, frame_name=None, verbose=None):
        capture = cam
        width = int(capture.get(cv2.CAP_PROP_FRAME_WIDTH))
        height = int(capture.get(cv2.CAP_PROP_FRAME_HEIGHT))
    ...
    def segmentAsAde20k(self, image_path, output_image_name=None, overlay=False, extract_segmented_objects=False,
        ...
        if overlay == True:
            alpha = 0.7
            frame_overlay = cv2.addWeighted(new_img, alpha, image_overlay, 1 - alpha, 0, image_overlay)

            if output_image_name is not None:
                new_image = cv2.rectangle(frame_overlay, (0.4 * w, 0.694 * h), (w * 0.6, h),
                                          (255, 0, 0), 2)
    ...
    def segmentFrameAsAde20k(self, frame, output_frame_name=None, overlay=False, verbose=None,
        ...
        if overlay == True:
            if extract_segmented_objects == False:
                ...
                segvalues, extracted_objects, frame_overlay = self.segmentAsAde20k(frame, overlay=True,
                                                                                   process_frame=True,
            ...
            if extract_segmented_objects == False:
                ...
                segvalues, extracted_objects, new_frame = self.segmentAsAde20k(frame, process_frame=True,
                                                                               extract_segmented_objects=True)
    ...
    def process_video_ade20k(self, video_path, overlay=False, frames_per_second=None, output_video_name=None,
        ...
        if overlay == True:
            ...
            while True:
                ...
                if ret:
                    if extract_segmented_objects == False:
                        segvalues, new_frame = self.segmentAsAde20k(frame, process_frame=True,
                                                                    extract_segmented_objects=False)
